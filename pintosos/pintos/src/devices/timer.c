...
/* sleeplist to calculate individual sleep times of threads*/
struct list sleeptime;
...
list_insert_ordered_define (struct list *list, struct list_elem* elemy)
 {
   ...
   for (e = list_begin (list); e != list_end (list); e = list_next (e)){
       ...
       if(thread_elemy->wakeuptime < thread_e->wakeuptime){
        break;
       }
   }
   return list_insert (e, elemy);
 }
/* function to calculate sleep time for current thread */
void sleep_until_calc(int64_t ticks){
  struct thread* current=thread_current();
  enum intr_level old_level=intr_disable();
  current->wakeuptime=ticks;
  list_insert_ordered_define(&sleeptime,&current->elem);
  thread_block();
  intr_set_level(old_level);
}
